...
export default function DNAQuests() {
	...
	// Load quests from localStorage or fallback to default
	const [quests, setQuests] = useState(() => {
			...
			: [
					{
						...
						progress: 0,
						max_progress: 5,
						deadline: "2025-09-28T23:59:59",
						active: true,
						target_trait: 10,
						reward: { description: "5 DNA Points", points: 5 },
						claimed: false,
					},
					{
						id: "2",
						...
						active: true,
						target_trait: 8,
						reward: { description: "4 DNA Points", points: 4 },
						claimed: false,
					},
					{
						id: "3",
						title: "Explore 3 Collections",
						...
						active: true,
						target_trait: 6,
						reward: { description: "2 DNA Points", points: 2 },
						claimed: false,
					},
					{
						id: "4",
	...
	// Save quests whenever updated
	useEffect(() => {
		localStorage.setItem("quests", JSON.stringify(quests));
	}, [quests]);

	// Save DNA points whenever updated
	useEffect(() => {
		localStorage.setItem("dnaPoints", dnaPoints);
	}, [dnaPoints]);
	...
	const updateProgress = (id) => {
		setQuests((prevQuests) =>
			prevQuests.map((quest) =>
				quest.id === id && quest.progress < quest.max_progress
	...
	const handleClaimReward = (quest) => {
		if (quest.claimed || quest.progress < quest.max_progress) return;
		setDnaPoints((prev) => prev + quest.reward.points);
		setQuests((prevQuests) =>
			prevQuests.map((q) => (q.id === quest.id ? { ...q, claimed: true } : q))
		);
		alert(🎉 You earned ${quest.reward.points} DNA Points!);
	};
						...
						<div
								...
								<div className="quest-info">
									...
									<div>
										...
										<div className="quest-meta">
											...
											<span>
												<Star className="small-icon" /> +{quest.reward.points}{" "}
												DNA
											</span>
										</div>
									</div>
							...
							<div className="quest-actions">
								{isCompleted && !quest.claimed ? (
									...
									<span className="claimed-label">Reward Claimed 🎉</span>
								) : isExpired ? (
									<span className="expired-label">Expired</span>
								) : (
									<button
										onClick={() => updateProgress(quest.id)}
										className="start-btn">
										Do Action ({quest.type})
									</button>
								)}
							</div>
						</div>
